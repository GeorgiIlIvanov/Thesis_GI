...
%\titleformat{\section}[display]
  %{\normalfont\centering}
  %{\huge\bfseries ---\thesection---}  % Number with lines on both sides
  %{0.2ex}                            % Reduced space between number and title
 % {\Large\bfseries}                 
%  [\vspace{1ex}\titlerule\vspace{1.5ex}] 
...
\subsection*{Non-Invariant Gaussian Model}
...
\end{cases}
\]
Our focus will be exclusively on the $d=1$ case. The primary motivation being that the Anderson model and the RBM are expected to have the same properties when $\lambda  \approx \frac{1}{W}$ \cite{7}, an observation supported with extensive numerical evidence \cite{arbm1, arbm2}. For localization, the current best bound is up to $W\ll N^{1/4}$ \cite{13}. On the other side of the transition, there is a long line of work of iterative improvements. Relevant examples include the work by Bourgade, Yang, Yau and Yin, which established for $W\gg N^{3/4}$ a strong form of delocalization and GUE statistics \cite{8, 9, 40}. Most recently, Dubova and Yang have rigorously proved quantum diffusion for Gaussian random band matrices, assuming $W\gg N^{8/11}$ \cite{bandSDE}. Sections \hyperref[sec:flow]{3} through \hyperref[sec:drift]{6} are dedicated exclusively to their work. The matrix model is defined as follows: identify $\Z_n$ with $\Gamma=\{1, \cdots, N\}$ and equip it with the aforementioned periodic distance $|i-j|_N$. Now, for a symmetric and compactly supported probability density function (PDF) on $\mathbb{R}$, define the doubly stochastic matrix as: $$S_{xy} := Z_{N, W}^{-1}f\left(\frac{|x-y|_N}{W}\right),$$
where $Z_{N, W}\asymp W$ is a normalizing constant bounded deterministically above and below by $W$. A technical, but necessary assumption is for $S$ to admit a matrix square root $S^{1/2}$, satisfying the same properties for a similarly symmetric and compactly supported PDF $f'$. 
...
\end{equation}
Observe that: $$m(z)(m(z)+z) = \frac{1}{4}(-z+\sqrt{z^2-4})(z+\sqrt{z^2-4})=\frac{1}{4}(-z^2+z^2-4)=-1$$
$$\Rightarrow |m(z)| = \frac{1}{|m(z)+z|^{-1}}$$
Now, take the imaginary part of $(2)$: $$\text{Im}(m(z)) + \text{Im}\left(\frac{1}{m(z)}\right)+\eta=0=\text{Im}(m(z)) + \frac{\text{Im}}{|m(z)|^2}+\eta$$
$$\Rightarrow \text{Im}(m(z))\left(1-\frac{1}{|m(z)|^2}\right)=-\eta$$
And since $\text{Im}(m(z))>0$ and $\eta>0$, we must have $1-\frac{1}{|m(z)|^2}<0$, i.e $|m(z)|<1$. 
$$\Rightarrow \text{Im }m \asymp 1\quad 1-|m|^2 \asymp \eta$$
...
\end{equation*}
Lastly, let us derive the derivatives of the resolvent w.r.t. to the matrix entry $H_{ij}$ using the limit definition. Let us define the perturbed matrix $H' = H(H_{ij}, H_{ji}\mapsto H_{ij}+\epsilon , H_{ji}+\epsilon)$ at entries $H_{ij}, H_{ji}$, where $G' = (H'-z)^{-1}$ and let $\epsilon_{ij} = \mathbf{0}(0_{ij}, 0_{ij}\mapsto \epsilon, \epsilon)$ be the perturbed zero matrix and write it as $\mathbf{1}_{ij}$ when $\epsilon=1$. Observe then that by our matrix identities, we have: $$\partial_{H_{ij}}G(z)_{xy}=\lim_{\epsilon\rightarrow 0} \frac{G_{xy}'-G_{xy}}{\epsilon} = \lim_{\epsilon\rightarrow 0}\frac{\left[G'(H-H')G\right]_{xy}}{\epsilon} = -\lim_{\epsilon\rightarrow 0}\frac{\left[G'\epsilon_{ij}G\right]_{xy}}{\epsilon} = -[G'\mathbf{1}_{ij}G]_{xy} = -G_{xi}G_{jy}$$

 $$\partial_{H_{ij}}\overline{G(z)_{xy}}=\lim_{\epsilon\rightarrow 0} \frac{\overline{G(z)_{xy}'}-\overline{G(z)_{xy}}}{\epsilon} = \lim_{\epsilon\rightarrow 0}\frac{\left[\overline{G'}(H^\intercal-H'^\intercal)\overline{G}\right]_{xy}}{\epsilon} = -\lim_{\epsilon\rightarrow 0}\frac{\left[\overline{G'}\epsilon_{ji}\overline{G}\right]_{xy}}{\epsilon} = -[\overline{G'}\mathbf{1}_{ji}\overline{G}]_{xy} = -\overline{G_{xj}}\overline{G_{iy}}$$
...
Lastly, carrying out the equivalent summation and using the fact that $(d(b)(t))^2=dt$, we get:
...
$$=\sum_{\alpha, \beta}G_{x\alpha}\sqrt{S_{\alpha \beta}}db_{\beta(t)}G_{\beta y}+\sum_j G_{xj}\left(S_{jy}(G_{yy}-m(z))\right)G_{jy}dt = dI(t) + dII(t)dt$$
This provides us with the decomposition of a stochastic term $dI(t)$ and a deterministic term $dII(t)dt$. 


\section*{Dynamical T- equation}
Now, define $F_{xy} = |G(t)_{xy}|^2 = G(t)_{xy}\overline{G(t)_{xy}}$. 
 Applying Itô's formula to \( F(t)_{xy} \), we have:
\[
dF(t)_{xy} = G(t)_{xy} d\overline{G(t)}_{xy} + \overline{G(t)}_{xy} dG(t)_{xy} + dG(t)_{xy} d\overline{G(t)}_{xy}.
\]
...
\]
On the other hand, by substituting \( dG(t)_{xy} = dI(t)_{xy} + dII(t)_{xy} dt \) and its conjugate, and noting that \( dI(t)_{xy} d\overline{I(t)}_{xy} \) contributes to the quadratic variation, we obtain:
\[
dF(t)_{xy} = dM(t)_{xy} + \Omega(t)_{xy} dt, \quad \text{where}
\]
\[
dM(t)_{xy} = \overline{G(t)}_{xy} dI(t)_{xy} + G(t)_{xy} \overline{dI(t)}_{xy}.
\]
$$
\Omega(t)_{xy} = G_{xy}\frac{\overline{dII(t)}_{xy}}{dt}+\overline{G_{xy}}\frac{dII(t)_{xy}}{dt}+\frac{d\left[G_{xy}, \overline{G_{xy}}\right]}{dt}=G_{xy}(t)\left(\overline{\sum_j G_{xj}(t)S_{jy}(G_{yy}(t)-m(z))G_{jy}(t)}\right)+$$
...
\end{equation}
Now, define $\Theta(t)=\Theta_t$ and $\epsilon_t(z)$ as:$$\Theta(t) = \frac{|m(z)|^2}{1-t|m(z)|^2S}\Leftrightarrow d\Theta_t = \Theta_t^2 dt$$
$$\E_t(z):T_t(z)-\Theta_t$$
$$\Rightarrow d\E_t(z) = T_t(z)^2dt + \Theta_t^2dt-S^{1/2}dM_t(z)S^{1/2}+S^{1/2}\Omega_t(z)S^{1/2}dt=$$
 $$=\{\Theta_t \E_t(z)+\E_t(z)\Theta_t\}dt +\E_t^2(z)dt - S^{1/2} dM_t(z)S^{1/2} + S^{1/2} \Omega_t(z) S^{1/2}dt$$
by observing that $T_t(z)^2-\Theta_t^2=\Theta_t\E_t(z)+\E_t(z)\Theta_t+\E_t(z)^2$. Integrating $\E_t(z)$ over $s$:
...
\subsection{Estimates for term $\mathcal{E}_t^{D}(z)_{ab}$} 
...
Following the regular vertex expansion, we have to now represent the subgraphs of the form $\mathcal{G}_{i2, s}=B_{u\gamma} S_{\gamma \delta} G_{x_i\delta} (G_{\gamma\gamma}-m)G_{\delta y_i} f(G)$, s.t. by applying the graphical representation we get: 
\begin{center}
\begin{tikzpicture}[
    thick, % Line thickness
    wavy/.style={decorate, decoration={snake, amplitude=2pt, segment length=5pt}},
...
\left[\{\text{Id} + (t-s)\Theta_t\}S^{1/2}\right]_{yb}
\end{equation*}
Here, we eliminate the loop at $\gamma$ and the edges $\delta\textcolor{blue}{\rightarrow}\beta$, $\beta\textcolor{blue}{\rightarrow}\alpha$ and $x\textcolor{red}{\rightarrow}y$, all contributing a term $O(W^{-1/2}\eta_s^{-1/4})$. By summing over $\beta\rightsquigarrow \alpha$ for an $O(1)$ term, we again have a bound that is equivalent to the previous ones, since all the wavy lines sum up to contribute $O(1)$. As such, we have a bound, equivalent to:
...
\subsection{Estimation of $\E_t^M(z)$ term}
...
$$\mathcal{E}_t^{M, 1} = \int_0^t(t-s)^2S^{1/2}\Theta_t\left\{\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\right\}S^{1/2}\Theta_t+\int_0^t(t-s)S^{1/2}\Theta_t\left\{\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\right\}S^{1/2}+$$
$$+\int_0^t(t-s)S^{1/2}\left\{\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\right\}S^{1/2}\Theta_t+ \int_0^t2S^{1/2}\left\{\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\right\}S^{1/2} =$$
...
\bibitem{40} F.Yang, H.-T. Yau, and J. Yin. Delocalization and quantum diffusion of random band matrices in high dimensions III: averaging fluctuations. \textit{Probability Theory and Related Fields}, 179: 451-540, 2021.
