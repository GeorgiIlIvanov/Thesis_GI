...
This means that by the Hölder's and the triangle inequalities: 
...
Using formula $(8.4)$ we found earlier gives us:
$$\mathcal{P} = \mathbb{E}\left\vert\left.\frac{d^6}{dt^6}F(tH_{ij})\right\vert_{t=t_{ij}}\right\vert$$

$$\lesssim \sum_{k=0}^6 |H_{ij}|^k \left\vert \partial_{H_{ij}}^\ell \partial_{H_{ji}}^{k-\ell} F(tH_{ij})|_{t=t_{ij}} \right\vert$$
