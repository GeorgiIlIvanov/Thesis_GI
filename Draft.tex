...
\noindent In our analysis, we will utilize the flow method (\hyperref[sec:flow]{Section 3}), where the spectral parameter $z=E+i\eta$ of the resolvent of $H$ (defined in \hyperref[sec:prereq]{Section 2}) is varied at constant-speed in the upper-half plane $\mathbb{H}$ ($E\in\mathbb{R}$ and $\eta>0$) as the entries of $H$ are realized as Brownian motion \cite{bandSDE}.  \textcolor{red}{After writing up Quantum Diffusion, justify assumption }$\eta\gg W^{-3/4}$ 
...
And since $\text{Im}(m(z))>0$ and $\eta>0$, we must have $1-\frac{1}{|m(z)|^2}<0$, i.e $|m(z)|<1$, which is equivalent to $\text{Im }m \asymp 1$, where $\asymp$ means being bounded above and below up to a fixed, positive factor. Furthermore, let us rewrite the latter equation as the following two identities: \begin{equation*}1-|m(z)|^2 = \frac{\eta|m(z)|^2}{\text{Im}(m(z))}
\tag{3.1}
\end{equation*}
\begin{equation*}\text{Im}(m(z))= \frac{\eta|m(z)|^2}{1-|m(z)|^2 }
\tag{3.2}
\end{equation*}
Now, let $E\in [-10, 10]$ and $\eta\in (0, 10]$. First observe that $m(z)$ can never be zero, since it implies: $$m(z) = \frac{-z+\sqrt{z^2-4}}{2}=0 \Leftrightarrow \sqrt{z^2-4}=z$$
$$\Rightarrow z^2-4=z^2\Leftrightarrow -4=0$$ 
Then, clearly $\exists c>0$, s.t. $|m(z)|\geq c$. \textcolor{red}{Write lemma A.1 here or transfer the latter to the appendix}.
...
\section{Flow Method}
\label{sec:flow}
Consider the Brownian perturbation of the standard deviation matrix defined as $H(t)_{xy} = \sqrt{S_{xy}}b(t)_{xy}$, where $b(t)\sim \mathcal{N}(0, t)$ represents the Brownian motion. For this system, the matrix resolvent is augmented as $G(t) = (H(t)-w(t))^{-1}$ , where $w(t) = -\frac{1}{m(z)}-tm(z)$. In order to study this system and its transition from $t=0$ with $G(0) = m(z)$, to $t=1$ with $G(1)=G(z)$, we will employ stochastic calculus. For this purpose, observe that our differentials are $dH(t)_{xy} = \sqrt{S_{xy}}db_{xy}(t)$ and $\partial_tw(t) = -m(z)$. Recall the Îto equation, namely, the statement that a function $f(\vec{b}(t), t)$, dependent on a Brownian random walk  $\vec{b}(t) = (b(t)_1, \cdots, b(t)_d)$ w.r.t to time $t$, has derivative:
...
Now, define $F_{xy} = |G(t)_{xy}|^2 = G(t)_{xy}\overline{G(t)_{xy}}$. 
...
To get the $T$-equation, define it as:
\[
T(t) =  S^{1/2}F(t)S^{1/2}=\sum_{x,y} S_{xy} F(t)_{xy} = \sum_{x,y} S_{xy} |G(t)_{xy}|^2,
\]
In order to find its derivative, we need to substitute for the earlier terms we found, namely \( dG(t)_{xy} = dI(t)_{xy} + dII(t)_{xy} dt \) and its conjugate, noting that \( dI(t)_{xy} d\overline{I(t)}_{xy} \) contributes to the quadratic variation, we obtain
\(
...
Where by Itô, the $T$-equation becomes: 
\begin{equation}
\Rightarrow d{T} = {T}^2 dt + S^{1/2} dM_t(z)S^{1/2} + S^{1/2} \Omega_t(z) S^{1/2}dt
\end{equation}
...
Let $\mathcal{E}_t(z)$ be the fluctuation term, i.e the difference between our $T$-matrix and the diffusion profile:
$$\E_t(z):T_t(z)-\Theta_t$$
Let us compute its evolution equation $d\E_t(z) = dT_t(z)-d\Theta_t$ by substituting for what we found before:
...
$$T_t(z)^2-\Theta_t^2 =(\E_t(z)+\Theta_t(z))^2- \Theta_t(z)^2=\E_t(z)^2+\Theta_t\E_t(z)+\E_t(z)\Theta_t$$
 ...
 $$\Rightarrow d\E_t(z)=\{\Theta_t \E_t(z)+\E_t(z)\Theta_t\}dt +\E_t^2(z)dt - S^{1/2} dM_t(z)S^{1/2} + S^{1/2} \Omega_t(z) S^{1/2}dt$$
Integrating $\E_t(z)$ over $s$:
$$\E_t(z)=\int_0^t \{\text{Id} + (t-s)\Theta_t\}\left\{\E^2(s)ds +S^{1/2} dM(s)S^{1/2}+  S^{1/2} \Omega(s) S^{1/2}\right\} \{\text{Id} + (t-s)\Theta_t\}ds,$$
whereas we can split the terms in three separate components, each of which we will individually bound in the following section:
...
\noindent The intuition behind the latter stopping times is simple - $\tau_{\sto, 1}$ reflects the first occurrence of 



The stopping times in equations (3.3)-(3.5) establish a bootstrapping mechanism that's fundamental to the proof. Let's analyze each component:

For $\tau_{\text{stop},1}$ in equation (3.3), we have:
\begin{align}
\tau_{\text{stop},1} := \inf\left\{s \geq 0 : \max_{a,b}|E_s(z)_{ab}| \geq W^{\delta_{\text{stop}}}W^{-\frac{3}{4}}|\text{Im}w_s|^{-1} \cdot W^{-1}|\text{Im}w_s|^{-\frac{1}{2}}\right\} \wedge 1
\end{align}

This controls the error term $E_s(z) = T_s(z) - \Theta_s$ between the actual flow $T_s(z)$ and the diffusion profile $\Theta_s$. The scaling factor $W^{-\frac{3}{4}}|\text{Im}w_s|^{-1} \cdot W^{-1}|\text{Im}w_s|^{-\frac{1}{2}}$ is carefully chosen for two reasons:

\begin{enumerate}
\item The typical size of $\max_{a,b}(\Theta_t)_{ab}$ is $W^{-1}|\text{Im}w_t|^{-\frac{1}{2}}$, which establishes the natural scale for comparison.

\item The additional factor $W^{-\frac{3}{4}}|\text{Im}w_s|^{-1}$ provides the necessary smallness to control nonlinear terms in the SDE flow for $E_s(z)$ (as seen in Lemma 7).
\end{enumerate}

The assumption $\eta \gg W^{-\frac{3}{4}}$ is critical because $\text{Im}w_t = \eta + (1-t)\text{Im}m(z)$ decreases monotonically from $\text{Im}w_0$ to $\text{Im}w_1 = \eta$. If $\eta$ were smaller than $W^{-\frac{3}{4}}$, then $|\text{Im}w_1|^{-1}$ would be too large to maintain control of $E_s(z)$, causing the error estimates to break down.

For $\tau_{\text{stop},2}$ in equation (3.4), we have:
\begin{align}
\tau_{\text{stop},2} := \inf\left\{s \geq 0 : \max_{a,b}\frac{|G_s(z)_{ab} - \delta_{ab}m(z)|^2}{(S^{1/2}T_s(z)S^{1/2})_{ab} + S^{1/2}_{ab} + W^{-D}} \geq W^{\delta_{\text{stop}}/10}\right\} \wedge 1
\end{align}

This controls the deviation of the resolvent entries from the semicircle law, normalized by appropriate terms. The denominator provides the natural scale for these deviations, with the addition of $W^{-D}$ ensuring regularity.

The self-reinforcing argument works as follows:

\begin{enumerate}
\item Initially, we know $E_0(z) = 0$, so $\tau_{\text{stop}} > 0$ with probability 1.

\item Assuming $s \leq \tau_{\text{stop}}$, we can control the error terms in the SDEs for $E_s(z)$ and $G_s(z)$.

\item These controlled estimates allow us to prove that $\mathbb{P}[\tau_{\text{stop}} < 1] \leq N^{-D}$ for any $D > 0$ (Theorem 8).

\item Therefore, with high probability, the error bounds hold for all $s \in [0,1]$, validating our assumptions.
\end{enumerate}

This is fundamentally a continuity argument: we assume the bounds hold up to $\tau_{\text{stop}}$, use them to prove $\tau_{\text{stop}} = 1$ with high probability, which in turn validates that the bounds hold throughout $[0,1]$.

The critical relationship between $W$ and $N$ (namely $W \gg N^{8/11}$) emerges precisely from ensuring that the error term $W^{-\frac{7}{4}}\eta^{-\frac{3}{2}}$ in equation (2.1) is much smaller than the typical size of $\Theta$ entries ($\approx W^{-1}\eta^{-\frac{1}{2}}$), while maintaining $\eta \approx W^2N^{-2}$, which corresponds to the diffusion time scale.


To further elaborate on the stopping time mechanism, we examine the error control in more detail:

\begin{enumerate}
\item The SDE for $E_t(z)$ in Lemma 7 contains nonlinear terms of the form $E_t(z)^2$. These terms require the smallness factor $W^{-\frac{3}{4}}|\text{Im}w_t|^{-1}$ to remain controlled.

\item When we solve the integral equation for $E_t(z)$, we obtain terms of the form $E_t^M(z)$, $E_t^D(z)$, and $E_t^S(z)$ (from the martingale part, the drift part, and the nonlinear self-interaction part, respectively).

\item The estimates in Lemmas 9, 10, and Proposition 11 show that each of these terms remains smaller than $W^{\delta_{\text{stop}}}W^{-\frac{3}{4}}|\text{Im}w_t|^{-1} \cdot W^{-1}|\text{Im}w_t|^{-\frac{1}{2}}$ with high probability.

\item This validates that $\tau_{\text{stop},1} = 1$ with high probability, which in turn implies that $\max_{a,b}|E_t(z)_{ab}| \prec W^{-\frac{7}{4}}|\text{Im}w_1|^{-\frac{3}{2}}$ (Theorem 2).
\end{enumerate}

The key insight is that the error bound from stopping time $\tau_{\text{stop},1}$ feeds into the estimates for $\tau_{\text{stop},2}$, allowing us to control $\max_{a,b}|G_t(z)_{ab} - \delta_{ab}m(z)|^2$ (Theorem 4). This demonstrates how the carefully chosen stopping times create a robust framework for establishing quantum diffusion and delocalization results.

The assumption $W \gg N^{8/11}$ arises from the need to ensure that:

\begin{align}
W^{-\frac{7}{4}}\eta^{-\frac{3}{2}} \ll W^{-1}\eta^{-\frac{1}{2}}
\end{align}

With $\eta \approx W^2N^{-2}$ (corresponding to the diffusion time scale), this inequality becomes:

\begin{align}
W^{-\frac{7}{4}}(W^2N^{-2})^{-\frac{3}{2}} \ll W^{-1}(W^2N^{-2})^{-\frac{1}{2}}\\
\Rightarrow W^{-\frac{7}{4}} \cdot W^{-3}N^{3} \ll W^{-1} \cdot W^{-1}N\\
\Rightarrow W^{-\frac{11}{4}}N^{3} \ll W^{-2}N\\
\Rightarrow W^{-\frac{3}{4}} \ll N^{-2}\\
\Rightarrow W \gg N^{\frac{8}{11}}
\end{align}

This demonstrates how the specific exponent $\frac{8}{11}$ arises naturally from the mathematical structure of the problem.
...
Assume that $|E|<2$ is fixed and that $\exists \nu>0$, s.t. $\eta \asymp W^2 N^{-2}$ and $W\geq N^{8/11+\nu}$. Fix any index $x$ and any integer $\ell\geq 1$. Define $P_{x, \ell}(y):=\mathbf{1}[|x-y|\geq \ell]$, i.e the projection on the complement of the open ball $B_\ell(x) = \{y:|x-y|<\ell \}$. Given any $\epsilon, \kappa>0$, define the labeling set for the localized to scale $\ell$ eigenvectors in the bulk as: $$\mathcal{A}_{\epsilon, \ell, \kappa} = \left\{\alpha: \lambda_\alpha \in [-E+\kappa, E-\kappa]: \sum_x |\mathbf{u}_\alpha(x)\|\right\}$$


\subsection{Quantum Diffusion}

Hence, our $\Theta$ term simplify $$\Theta \sim \frac{S}{1-|m(z)|^2+|m(z)|^2(S-\text{Id})}\sim \frac{S}{\alpha\eta+(S-\text{Id})}$$
...
There will be two important observations used throughout the following estimations. First, per the covariance normalization, ofthe generalized Wigner matrix \cite{dynamic} , we have $S_{ab}\lesssim N^{-1}$. Secondly, $\sum_{y}|B_{xy}|$, $\sum_{y}|S_{xy}|$ have size $O(1)$ per Lemma 25 \cite{bandSDE}. Now, having established that, we can use estimate $(1.3)$ from the Lemma above, along with resolvent identity $(3)$, s.t. we get:
...
\noindent Then for $M :=\left(\max_{i, j}S_{i,j}\right)^{-1}\gtrsim N$, by Lemma 3.8 from \cite{knowles2013}, we have that:
...
\label{sec:drift}
\textit{Give backstory} \cite{RP, soosten}.
...
\vspace{0.25 cm}
\begin{boxt}{Diagrammatic notation}
\begin{itemize}
...
$$\int_0^t \{\text{Id} + (t-s)\Theta_t\} S^{1/2}dM(s)S^{1/2} \{\text{Id} + (t-s)\Theta_t\}= $$$$=\int_0^t \{\text{Id} + (t-s)\Theta_t\}\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\{\text{Id} + (t-s)\Theta_t\}+$$
$$+ \int_0^t \{\text{Id} + (t-s)\Theta_t\}\sum_{u, v}\left[G_{xy}\overline{G_{xu}dH_{uv}(t)}\overline{G_{vy}}\right]\{\text{Id} + (t-s)\Theta_t\}$$

\noindent After rewriting the fractions as $\left[\text{Id}+(t-s)\Theta_t\right]_{ax}$ and with index $yb$, respectively, the latter two terms can be written in diagrammatically using our familiar notation:
\begin{center}
\begin{tikzpicture}[
    thick, % Line thickness
    wavy/.style={decorate, decoration={snake, amplitude=2pt, segment length=5pt}},
...
\end{tikzpicture}
\end{center}
Now, if we are to approach the bounding carelessly, we will quickly stumble into problems. For example, by applying our standard estimates, first eliminating the wavy line, from which we have $\sqrt{S_{uv}}\lesssim N^{-1/2}$ per estimate $4$ and another $O(\sqrt{dt})$ per the fact that $\mathbb{E}\left\vert dB_{uv}(t)\right\vert^2=dt$. We want to keep a connected component similar to before, so let WLOG this be the path $x'\Rightarrow a\Rightarrow x\textcolor{blue}{\rightarrow }y\Rightarrow b\Rightarrow y'$. By applying the already familiar estimate $2$ to the 4 red lines, we get a combined term $O(N^{-2}\eta_s^{-1})$. Now, to get to the the aforementioned path, we need to bound the $x'\textcolor{blue}{\rightarrow}y'$, using estimate 1 (since the edge is not "free") with which we get $O(N^{1/2}\eta_s^{-3/4})$. By observing what remains, we can quickly see this will prove to be unruly due to the fact that we will have to split double lines into the respective $\text{Id}$ and $(t-s)\Theta_t$ components, yielding $2^3$ separate terms, which will each be of the form $O\left(N^{-\frac{3}{2}}\eta_t^{-\frac{3a+2b}{2}}\eta_{s}^{\frac{5a+3b}{4}}\right)$, where $a, b\in \{0, 1, 2, 3\}$ and $a+b=3$ (we have an additional estimate from the blue line that is $N^{1/2}\eta_s^{-3/4}$). We can easily observe that by accounting for all of the terms, we have in the worst case scenario a term $\eta_s^{15/4}$, which remains as $\eta_s^{5/4}$ after canceling the other such terms. By the same logic, we can observe that we have at most $\eta_t^{-\frac{9}{2}}$ and at least $\eta_t^{-3}$ from the splitting terms, hence the bound quickly becomes unruly as we take the limit $\eta_t\rightarrow 0$. 


\subsubsection*{Alternative bound}
The latter calculations demonstrate that we need to be more thoughtful with our estimates. Let us come back to our original form of the $S^{1/2}dM(s)S^{1/2}$ term:
$$=\int_0^t\left[\text{Id}+(t-s)\Theta_t\right]S^{1/2}\left\{\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\right\}S^{1/2}\left[\text{Id}+(t-s)\Theta_t\right] +$$
$$\int_0^t \left[\text{Id}+(t-s)\Theta_t\right]S^{1/2}\left\{\sum_{u, v}\left[G_{xy}\overline{G_{xu}dH_{uv}(t)}\overline{G_{vy}}\right]\right\}S^{1/2}\left[\text{Id}+(t-s)\Theta_t\right] = \mathcal{E}_t^{M, 1}+\mathcal{E}_t^{M, 2}$$ 
Observe that we can split each of the components into 4 separate parts by simply expanding the square brackets. We will this explicitly for $\mathcal{E}_{t}^{M, 1}$, whereas the second term is equivalent up to complex conjugation:

$$\mathcal{E}_t^{M, 1} = \int_0^t(t-s)^2S^{1/2}\Theta_t\left\{\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\right\}S^{1/2}\Theta_t+$$
$$+\int_0^t(t-s)S^{1/2}\Theta_t\left\{\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\right\}S^{1/2}+$$
$$+\int_0^t(t-s)S^{1/2}\left\{\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\right\}S^{1/2}\Theta_t+$$
$$+ \int_0^t2S^{1/2}\left\{\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\right\}S^{1/2} =$$
$$= \mathcal{E}_t^{M, 11}+\mathcal{E}_t^{M, 12}+\mathcal{E}_t^{M, 13}+\mathcal{E}_t^{M, 14}$$
Now, instead of applying the BDG inequality to the entire term, we can bound the individual $\mathcal{E}_t^{M, 1i}$ entries:

$$\mathbb{E}\left\vert \mathcal{E}^{M, 1i}_{ab}(s)\right\vert^{2p}\leq C_p\mathbb{E}\left[ \mathcal{E}^{M, 1i}_{ab}(s) \right]_t^p$$

\noindent Hence, we need to calculate the quadratic variation for each term. To do this, recall from before that the Brownian increments $dB_{\alpha\beta}(t)$ are mutually uncorrelated, i.e our term $dH_s = \sum_{u, v} \sqrt{S_{u, v}}dB_{u, v}(s)$ has covariance structure: $$\mathbb{E}\left[dH_s\overline{dH_s}\right] = \mathbb{E}\left[\sum_{u, u', v, v'}\sqrt{S_{uv}}dB_{uv}(s)\sqrt{S_{u'v'}}dB_{u'v'}(s)\right]_{ab}=S_{ab}ds$$
Hence the QV of each of the latter terms is:
$$\Rightarrow\left[\mathcal{E}^{M, 11}\right]_{ab} = \int_0^t(t-s)^4\sum_{x, x', y, y', u, v}(\Theta_tS^{1/2})_{ax}(\Theta_tS^{1/2})_{ax'}\overline{G_{xy}}G_{x'y'}G_{xu}\overline{G}_{x'u}S_{uv}G_{vy}\overline{G_{vy'}}(S^{1/2}\Theta_t)_{yb}(S^{1/2}\Theta_t)_{yb'}ds$$

$$\left[\mathcal{E}^{M, 12}\right]_{ab} = \int_0^t(t-s)^2\sum_{x, x', y, y', u, v}(\Theta_tS^{1/2})_{ax}(\Theta_tS^{1/2})_{ax'}\overline{G_{xy}}G_{x'y'}G_{xu}\overline{G}_{x'u}S_{uv}G_{vy}\overline{G_{vy'}}S^{1/2}_{yb}S^{1/2}_{yb'}ds$$

$$\left[\mathcal{E}^{M, 13}\right]_{ab} = \int_0^t(t-s)^2\sum_{x, x', y, y', u, v}S^{1/2}_{ax}S^{1/2}_{ax'}\overline{G_{xy}}G_{x'y'}G_{xu}\overline{G}_{x'u}S_{uv}G_{vy}\overline{G_{vy'}}(S^{1/2}\Theta_t)_{yb}(S^{1/2}\Theta_t)_{yb'}ds$$

$$\left[\mathcal{E}^{M, 14}\right]_{ab} = \int_0^t\sum_{x, x', y, y', u, v}S^{1/2}_{ax}S^{1/2}_{ax'}\overline{G_{xy}}G_{x'y'}G_{xu}\overline{G}_{x'u}S_{uv}G_{vy}\overline{G_{vy'}}S^{1/2}_{yb}S^{1/2}_{yb'}ds$$

\noindent Observe that we have the same primary components of the separate terms, differing up to the utmost left and right terms. Hence, we can abstract away four separate matrices, to represent this two-by-two  overlap: 
...
$$
$$ \left[\mathcal{E}_{t}^{M, 11}(z)_{ab}\right] = \int_0^t (t-s)^4\sum_{u, y} (\Omega^u\Upsilon^{u, *})_{yy'}ds \ , \ \left[\mathcal{E}_{t}^{M, 12}(z)_{ab}\right] = \int_0^t (t-s)^2\sum_{u, y} (\Omega^u\Gamma^{u, *})_{yy'}ds$$
$$\Rightarrow \left[\mathcal{E}_{t}^{M, 13}(z)_{ab}\right] = \int_0^t \sum_{u, y} (t-s)^2(\Xi^u\Upsilon^{u, *})_{yy'}ds \ , \ \left[\mathcal{E}_{t}^{M, 14}(z)_{ab}\right] = \int_0^t \sum_{u, y} (\Xi^u\Gamma^{u, *})_{yy'}ds$$
Now, observe that both $\Omega^u$ and $\Xi^u$ are positive-semidefinite, since we can rewrite them as:
...
This means that we can apply the trace inequality, which states that the trace is bounded above by singular values $\left\vert\text{Tr}(AB)\right\vert\leq \sum_{i=1}^N \alpha_i\beta_i$, hence by the positive-semidefiniteness, we have that $\left\vert\sum_y\left(\Omega^u\Upsilon^{u, *}\right)_{yy}\right\vert = \left\vert\text{Tr}\left(\Omega^u\Upsilon^{u, *}\right)\right\vert\leq \left\|\Omega^u\right\|_{op}\sum_{y}\Omega_{yy'}^uds$. This means that we can bound each of the earlier terms as follows:

$$\left[\mathcal{E}_{t}^{M, 11}(z)_{ab}\right] = \int_0^t (t-s)^4\sum_{u, y} (\Omega^u\Upsilon^{u, *})_{yy}ds\leq \int_0^t (t-s)^4\sum_u \|\Upsilon^u\|_{op}\sum_y \Omega_{yy'}^uds$$

$$\left[\mathcal{E}_{t}^{M, 12}(z)_{ab}\right] = \int_0^t (t-s)^2\sum_{u, y} (\Omega^u\Gamma^{u, *})_{yy'}ds\leq \int_0^t (t-s)^2\sum_u \|\Gamma^u\|_{op}\sum_y \Omega_{yy'}^uds$$

$$\left[\mathcal{E}_{t}^{M, 13}(z)_{ab}\right] = \int_0^t \sum_{u, y} (t-s)^2(\Xi^u\Upsilon^{u, *})_{yy'}ds \leq  \int_0^t (t-s)^2\sum_u \|\Upsilon^u\|_{op}\sum_y \Xi_{yy'}^uds$$

$$\left[\mathcal{E}_{t}^{M, 14}(z)_{ab}\right] = \int_0^t \sum_{u, y} (\Xi^u\Gamma^{u, *})_{yy'}ds\leq \int_0^t \sum_u \|\Gamma^u\|_{op}\sum_y \Xi_{yy'}^uds$$
...
following from the Schwarz inequality. By the Ward's identity for the resolvent $|G_{xx}|^2 =  \frac{\text{Im}G_{xx}}{\eta}=\frac{\text{Im}G_{xx}}{\text{Im}w_s}$ (equation 7.23 in \cite{dynamic}), we have:

$$4 \sum_{x}|G|_{xx}^4 |(\Theta_t S^{\frac{1}{2}})_{ax}|^2=4 |\text{Im}w_s|^{-2}\sum_x|\text{Im}G_{xx}|^2|(\Theta_t S^{\frac{1}{2}})_{ax}|^2\leq 4 |\text{Im}w_s|^{-2}|\max_{k}\text{Im}G_{xx}|^2\sum_x|(\Theta_t S^{\frac{1}{2}})_{ax}|^2\leq$$

$$\textcolor{red}{?\lesssim} |\text{Im} w_s|^{-3}\max_k \text{Im}G_{kk}\sum_{x} |(\Theta_t S^{\frac{1}{2}})_{ax}|^2\leq N^{-1}|\text{Im}w_s|^{-3}|\text{Im}w_t|^{-3/2},$$

using estimates $(4)$ and $(5)$. By the same logic, we have that: $$\sum_{u, y}\Xi_{yy}^u=\sum_{u,y}\sum_{x,x'} \overline{G}_{xy} G_{x'y'} G_{xu} \overline{G}_{x'u} S^{\frac{1}{2}}_{ax} S^{\frac{1}{2}}_{ax'} = \sum_{x, x'}|G|^2_{xx'}|{G}|^2_{x'x} S^{\frac{1}{2}}_{ax} S^{\frac{1}{2}}_{ax'}\leq $$
$$\leq 4 \sum_{x}|G|_{xx}^4 |S^{\frac{1}{2}}_{ax}|^2\lesssim |\text{Im}w_s|^{-3}\max_k\text{Im}G_{kk}\sum_x|S_{ax}^{1/2}|^2\lesssim N^{-1}|\text{Im}w_s|^{-3}$$
Lastly, we need to bound the other two matrices, which we can do by using the fact that the diagonal matrix $S^u$ is of the same order as estimate $6$, i.e by bounding with the maximal value and Wald's identity, we have: 
$$\|\Upsilon^u\|_{op}\leq \max_{\alpha, \beta}|(\Theta_t S^{1/2})|^2 N^{-1}|\text{Im}w_s|^{-2}\lesssim N^{-1}|\text{Im}w_s|^{-2}|\text{Im}w_t|^{-1}$$

$$\|\Gamma^u\|_{op}\leq\max_{\alpha,\beta}|S_{\alpha\beta}^{1/2}|^2N^{-1}|\text{Im}w_s|^{-2}\lesssim N^{-3}|\text{Im}w_s|^{-2}$$
Lastly, recall estimate 9 that $(t-s)\lesssim |\text{Im}w_s|^{-1}$, with which we have all the necessary pieces to complete the estimation of all the QV terms $\mathcal{E}^{M, 1i}_t$, by again changing the variable of integration w.r.t $\sigma = |\text{Im}w_s|$:


$$\left[\mathcal{E}_{t}^{M, 11}(z)_{ab}\right] \leq \int_0^t (t-s)^4\sum_u \|\Upsilon^u\|_{op}\sum_y \Omega_{yy'}^uds \lesssim N^{-4}|\text{Im}w_t|^{-5/2}\int_{\text{Im}w_t}^0\sigma^{-1}\lesssim N^{-4}|\text{Im}w_t|^{-5/2}\log|\text{Im}w_t| $$
...
\label{La1}
\textbf{Lemma A.1} $$ 1-|m|^2 \asymp \eta$$ (Lemma 3.5 \cite{21} and Lemma 6.2 \cite{dynamic})
...
\bibitem{knowles2013} L. Erdős, A. Knowles, H.-T. Yau, and J.Yin. Delocalization and Diffusion Profile for Random Band Matrices. \textit{Communications in Mathematical Physics}, 323: 367-416, 2013.
