...
Lastly, we need to integrate over $s\in [0, t]$ by using a change of variable $\sigma =\eta_s$, s.t. the bounds of the integral are then $\sigma\in [\eta_t, O(1)]$, i.e we have:
$$\max_{i=1, 2, 3, 4} \int_0^{t\wedge \tau_\sto}|\mathcal{G}_{i0, s}|ds\lesssim W^{-2}\eta_t^{-3/2} \int_{\eta_t}^{O(1)}d\sigma + W^{-2}\eta_t^{-1}\int_{\eta_t}^{O(1)}\sigma^{-1/2}d\sigma \lesssim$$
