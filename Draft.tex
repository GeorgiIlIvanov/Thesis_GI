...
\item For any $s \leq \tau_{\text{stop}}$, we can control the error terms in the flow SDE for $E_s(z)$ and $G_s(z)$ by means of martingale inequalities. 


\item These estimates will allow us to prove that $\mathbb{P}[\tau_{\text{stop}} < 1] \leq N^{-D}$ for any $D > 0$.


\item Therefore, the error bounds hold for all $s \in [0,1]$, allowing us to prove that $\tau_\sto =1$ with high probability. 

\end{enumerate}

\noindent The particular delocalization bound for $W \gg N^{8/11}$ emerges from the need of ensuring that the combined error term $W^{-\frac{7}{4}}\eta^{-\frac{3}{2}}$ in equation $(3.2.1)$ is much smaller than the typical size of the $\Theta$ entries of $O( W^{-1}\eta^{-\frac{1}{2}})$, while maintaining he diffusion time scale $\eta \approx W^2N^{-2}$ (\hyperref[diffusion]{Sec. 4.2}) :
