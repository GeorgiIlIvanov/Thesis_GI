...
\end{center}
\begin{center}
\justify{\textsc{First, and foremost}, I would like to thank Kevin Yang, without whom this thesis would not have been possible. I could not have asked for a more supportive advisor - one who allowed me to explore ideas freely, whilst nudging me towards concrete steps that  gave them shape.\vspace{0.25 cm} 
...
\noindent \textsc{I want to thank the friends that made this foreign land more hospitable:}\\
...
\noindent \textsc{I am grateful for my friends back home} - Stani, Eli, Lily and Georgi - for being by my side throughout the years, unwavering in their belief and unapologetic in their support.\vspace{0.25 cm}
...
\tag{2.3}
\end{equation*}
Now, let $E\in [-10, 10]$ and $\eta\in (0, 10]$. First, we claim that $|m(z)|\geq c>0$ uniformly. Indeed, $m(z)$ is continuous, this is equivalent to $m(z)\neq 0$, which can be verified as follows: $$m(z) = \frac{-z+\sqrt{z^2-4}}{2}=0 \Leftrightarrow \sqrt{z^2-4}=z$$
...
In order to find the derivative of the $T$-matrix, we need to substitute for the earlier terms we found, namely \( dG(t)_{xy} = d\mathrm{I}(t)_{xy} + d\mathrm{II}(t)_{xy} dt \) and its conjugate, noting that \( d\mathrm{I}(t)_{xy} d\overline{\mathrm{I}(t)}_{xy} \) contributes to the quadratic variation, we obtain
\(
...
\end{equation*}
and when expanded w.r.t to $d\mathrm{I}(t)$ and $d\mathrm{II}(t)dt$:
...
Therefore, by the pathwise uniqueness of solutions to SDEs (Thm. 9.1 \cite{steele}), the following expression satisfies the same SDE:\textcolor{red}{Define:
...
In this section we will state all of the inequalities and stochastic bounds we will need for control of our SDE terms. Their rigorous justification is presented in the \hyperref[sec:appendix]{Appendix}. From now on, for the sake of brevity we will define $\eta_s = |\im w_s|$ and $\eta_t = |\im w_t|$, the notation of which includes the indexing with any particular time instantiation $t_0\in [0, 1]$. With this, let us state
...
\end{equation*}
\end{center}
We can apply estimate $(2)$ to the two loops around $\alpha$ and $\beta$, as well as estimate $(1)$ to the red edge connecting $x$ and $y$. From this, we receive three multiplicative terms $O(W^{\delta_\sto/20}W^{-1/2}\eta_s^{-1/4})$, i.e combined it yields a term $O(W^{3\delta_\sto/20}W^{-3/2}\eta_s^{-3/4})$. By estimate $(5)$, we can sum all the wavy lines out in order $\beta, \alpha, v,$ and finally $u$ for a $O(1)$ term, keeping our earlier estimate. We can stop here by leaving the connected component that is a double line from $a$ to $x$, a blue line from $x$ to $y$ and another double line from $y$ to $b$, i.e: $\mathcal{G}_{10, s}\lesssim W^{3\delta_\sto/20}W^{-3/2}\eta_s^{-3/4}\times $
\begin{tikzpicture}[
    thick, % Line thickness
    wavy/.style={decorate, decoration={snake, amplitude=2pt, segment length=5pt}},
    ]
...
\end{equation*}
\end{center}
Here, we eliminate first the edges  $\beta\textcolor{red}{\rightarrow}\alpha$ and $\alpha\textcolor{blue}{\rightarrow} y$ using estimate $(2)$, giving us a $O(W^{\delta_\sto/10}W^{-1}\eta_s^{-1/2})$ term. Due to the edge from $x\textcolor{blue}{\rightarrow}\beta$, we are not going to be summing $\alpha\leadsto\beta$ over $S_{ab}$ entries because it serves as a multiplicative term to the blue edge $x\textcolor{blue}{\rightarrow}\beta$. For this reason, we will apply the individual $S_{ab}\lesssim W^{-1}$ bound, s.t now summing over $x\textcolor{blue}{\rightarrow}\beta$ with estimate $(1)$ gives us a combined bound $O(W^{\delta_\sto/20}W^{\frac{1}{2}+\epsilon}\eta_s^{-3/4})$. Finally, by summing over the remaining wavy edges, starting with $\alpha, v$ and then $u$, due to the $O(1)$ term (estimate $(5)$), we get an estimate of the form: $\mathcal{G}_{30, s}\lesssim W^{3\delta_\sto/20}W^{-\frac{1}{2}+\epsilon}\eta_s^{-5/4}\times $
\begin{tikzpicture}[
    thick, % Line thickness
    wavy/.style={decorate, decoration={snake, amplitude=2pt, segment length=5pt}},
    ]
...
$}\end{equation*}
\end{center}
\noindent To estimate this graph, we will bound all loops $\textcolor{blue}{\circlearrowright}$ and $x\textcolor{red}{\rightarrow} y$ using estimate $(2)$ which yields in total $O(W^{\delta_\sto/5}W^{-2}\eta_s^{-1})$. Then, we sum the wavy lines over $\beta, \alpha, v, u, \delta$ in that order, which is $O(1)$ by estimate $(5)$.The combined bound is:
...
\noindent Here, we will repeat an equivalent procedure as above, bounding the loop at $\delta$ and the edges $x\textcolor{red}{\rightarrow}\alpha$, $\beta\textcolor{blue}{\rightarrow}\alpha$ and $\beta\textcolor{red}{\rightarrow}y$ , getting a bound $O(W^{\delta_\sto/5}W^{-2}\eta_s^{-1})$. Then, we can sum the wavy lines with estimate $(5)$, getting an equivalent bound as above:
...
\noindent Let us start by bounding the loop at $\delta$, as well as the $x\textcolor{red}{\rightarrow}y$, $\beta\textcolor{blue}{\rightarrow}\alpha$ and $\alpha\textcolor{blue}{\rightarrow}\gamma$. In total, this contributes $O(W^{\delta_\sto/5}W^{-2}\eta_s^{-1})$. However, here we don't have the same structure as before, since $\gamma$ no longer has an ingoing edge from $x$. We can abstract away the wavy lines (blue and red) with an orange wavy line, and handle the individual terms, which will be contributing only with $O(1)$ per estimate $(5)$. As such, the bound is of the form: $\mathcal{G}_{31, s}\lesssim W^{\delta_\sto/5} W^{-2}\eta_s^{-1}\times $
\begin{tikzpicture}[
    thick, % Line thickness
    wavy/.style={decorate, decoration={snake, amplitude=2pt, segment length=5pt}},
    ]
...
where $H_{N, N}^G, \cdots, H_{i, j+1}^G,  H_{ij}^G$ are the Gaussian replacement entries, matching in the sense of $(8.1)$ :

\begin{boxt}{Conditions}
\begin{enumerate}[label=(8.\arabic*)]\label{8.1}
\item $\mathbb{E}\left[H_{ij}^\ell \left(\overline{H}_{ij}\right)^{k-\ell}\right] = \mathbb{E}\left[\left(H_{ij}^G\right)^\ell \left(\overline{H}_{ij}^G\right)^{k-\ell}\right], \quad 0\leq \ell\leq k\leq 5$\label{8.2}
\item $\forall q\geq 1, \quad \mathbb{E}\left\vert H_{xy}\right\vert ^{2q}\prec W^{-q}$
\end{enumerate}
\end{boxt}
The first condition $(8.1)$ is the $\textit{Five Moment}$ matching assumption that gives the name of our result. The second $(8.2)$ is a moment decay condition that allows us to relax the Gaussian assumption, while still providing sufficient strength of our bounds. However, we still have to verify that each consecutive replacement is manageable. For this purpose, we will define the interpolation for $t\in [0, 1]$ at entries $(ij)$ and $(ji)$, where with slight abuse of notation, we let:
...
by condition $(8.1)$. This means that:
...
where the first is the triangle inequality, the second is Hölder's $L^1$ inequality and the last step uses the fact that $|H_{ij}| = \left\vert\overline{H_{ij}}\right\vert$. Let us call the RHS $\mathcal{Q}$, i.e we just showed that $\mathcal{P}\lesssim \mathcal{Q}$. WLOG, we also have that $\mathcal{P}^G\lesssim \mathcal{Q}^G$. By condition $(8.2)$, we have that $\mathbb{E}|H_{ij}|^6\prec W^{-3}$, so our goal will be to establish a bound on the first multiple term, namely $\left\vert \partial_{H_{ij}}^\ell \partial_{H_{ji}}^{k-\ell} F(tH_{ij})\right\vert$ for all $t\in [0, 1]$. The delocalization argument will follow readily, so let us now direct our attention to the derivatives on $F(H)$ w.r.t. the entries $H_{ij}$. 

\subsection{Derivatives of $F(tH_{ij})$}
By looking at the general form of our target $\left\vert \partial_{H_{ij}}^\ell \partial_{H_{ji}}^{k-\ell} F(tH_{ij})\right\vert$ , we can glean that a good place to start would be with the calculation of $\partial_{H_{k\ell}}F(tH_{ij})$. Given our definition of the interpolation $tH_{ij}$, we can tell in advance that term $t$ will only appear as a multiple after we apply the chain rule, so we can focus exclusively on $\partial_{H_{xy}}^\ell F(tH_{xy})$. Recall our definition $(8.3)$ of $F(H)$: 
$$F(H) = \left\vert
(H-z)_{ij}^{-1} - m(z)S_{ij}
\right\vert ^{2p} = \left\vert 
G_{ij}(z)-m(z)S_{ij}
...
$$
Let us apply the chain rule, by recognizing that $|W_{ij}|^{2p} = \left(W_{ij} \cdot \overline{W_{ij}}\right)^p$ :

$$\frac{\partial |W_{ij}|^{2p}}{\partial H_{kl}} = \frac{\partial |W_{ij}|^{2p}}{\partial W_{ij}} \cdot \frac{\partial W_{ij}}{\partial G_{ij}} \cdot \frac{\partial G_{ij}}{\partial H_{kl}} + \frac{\partial |W_{ij}|^{2p}}{\partial \overline{W_{ij}}} \cdot \frac{\partial \overline{W_{ij}}}{\partial \overline{G_{ij}}} \cdot \frac{\partial \overline{G_{ij}}}{\partial H_{kl}}$$
Using complex differentiation, we have:
$$\frac{\partial |W_{ij}|^{2p}}{\partial W_{ij}} = p|W_{ij}|^{2p-2} \cdot \overline{W_{ij}}, \quad\frac{\partial |W_{ij}|^{2p}}{\partial \overline{W_{ij}}} = p|W_{ij}|^{2p-2} \cdot W_{ij}$$
And since $W_{ij} = G_{ij} - mS_{ij}$:
$$\frac{\partial W_{ij}}{\partial G_{ij}} = 1, \quad \frac{\partial \overline{W_{ij}}}{\partial \overline{G_{ij}}} = 1$$
Recall the resolvent identities we proved in \hyperref[resolventderivative]{Section 2}, namely $\partial_{H_{ij}}G_{xy} = -G_{xi}G_{jy}$ and $\partial_{\overline{H_{ij}}}\overline{G_{xy}} = -\overline{G_{xi}}\ \overline{G_{jy}}$ . Substituting the derivatives into our chain rule expression yields:

$$\frac{\partial |W_{ij}|^{2p}}{\partial H_{kl}} = p|W_{ij}|^{2p-2} \cdot \overline{W_{ij}} \cdot 1 \cdot (-G_{ik}G_{lj}) + p|W_{ij}|^{2p-2} \cdot W_{ij} \cdot 1 \cdot (-\overline{G_{ik}}\overline{G_{lj}})$$
$$\frac{\partial |W_{ij}|^{2p}}{\partial H_{kl}} = -p|W_{ij}|^{2p-2} \cdot [\overline{W_{ij}} \cdot G_{ik}G_{lj} + W_{ij} \cdot \overline{G_{ik}G_{lj}}]$$
$$\frac{\partial |W_{ij}|^{2p}}{\partial H_{kl}} = -p|W_{ij}|^{2p-2} \cdot [\overline{W_{ij}} \cdot G_{ik}G_{lj} + (\overline{W_{ij}} \cdot G_{ik}G_{lj})^*]$$
$$\frac{\partial |W_{ij}|^{2p}}{\partial H_{kl}} = -2p|W_{ij}|^{2p-2} \cdot \text{Re}(\overline{W_{ij}} \cdot G_{ik}G_{lj})$$
Hence, by putting everything together, we get:
...
The first term is: $$\partial_{H_{k\ell}}\left[|W_{ij}|^{2p-2}\right] = (2p-2)|W_{ij}|^{2p-4}\partial_{H_{k\ell}}\left[|W_{ij}|^2\right] = (2p-2)|W_{ij}|^{2p-4}\left(\partial_{H_{k\ell}} W_{ij}\cdot \overline{W}_{ij}+W_{ij}\cdot \partial_{H_{k\ell}} \overline{W}_{ij}\right)=$$
...
And the second is: $$\partial_{H_{k\ell}}\left[\text{Re}\left(W_{ij}\overline{G_{ik}G_{\ell j}}\right)\right] = \text{Re}\left(\partial_{H_{k\ell}} W_{ij} \overline{G_{ik}G_{\ell j}}+W_{ij} \partial_{H_{k\ell}} \overline{G_{ik}G_{\ell j}}\right)=$$
...
As such, by combining the terms, we get the following expansion: 
...
While this doesn't look as bad, we have already went from one to four terms in our expression (two and eight, respectively, if written in conjugate form). In fact, the growth is factorial, giving that the chain rule generates several new elements for each term: 
\[\partial_{H_{k\ell}}^3F(H)  = -48p(p-1)(p-2)|W_{ij}|^{2p-6}\text{Re}^3\left(W_{ij}\overline{G_{ik}G_{\ell j}}\right)+\]
$$+24p(p-1)|W_{ij}|^{2p-4}\text{Re}\left(W_{ij}\overline{G_{ik}G_{\ell j}}\right)\text{Re}\left(W_{ij}\overline{G_{ii}G_{kk}G_{\ell j}}+W_{ij}\overline{G_{ik}G_{\ell \ell}G_{jj}}\right)-$$
$$-4p(2p-2)|W_{ij}|^{2p-4}\text{Re}\left(W_{ij}\overline{G_{ik}G_{\ell j}}\right)|G_{ik}G_{\ell j}|^2-2p|W_{ij}|^{2p-2}\text{Re}\left(W_{ij}\overline{G_{ik}G_{\ell i}G_{kk}G_{\ell j}}\right)-$$
...
\[-2p|W_{ij}|^{2p-2}\text{Re}\left(W_{ij}\overline{G_{ik}G_{\ell k}G_{\ell\ell}G_{jj}}\right)-2p|W_{ij}|^{2p-2}\text{Re}\left(W_{ij}\overline{G_{ik}G_{\ell \ell}G_{jk}G_{\ell j}}\right)-\]
...
$$-2p|W_{ij}|^{2p-2}\text{Re}\left(G_{ik}G_{\ell j}\overline{G_{ii}G_{kk}G_{\ell j}}+G_{ik}G_{\ell j}\overline{G_{ik}G_{\ell \ell}G_{jj}}\right)-$$
We can see that this quickly becomes unruly. Getting to exponent six, which is what we need for the bound on $\mathcal{Q}$, will simply be impossible without some way of abstracting away the details. Fortunately, we are only concerned with stochastic bounds, and as such, don't need the exact form of the terms, only their approximate size. 

\subsection*{Abstract Form}
From our earlier estimates it becomes clear that calculating an explicit formula would be rather troublesome. As such, we should instead try and abstract away the derivative structure, demonstrating that even with crude stochastic estimates, there is a form of continuity argument that retains our control over the entire replacement process. Having calculated up to the third derivative, we can now make an educated guess as to the form that all subsequent steps would take. Namely, we should expect: 

\begin{equation*}
\partial_{H_{ij}}^{k_1}\partial_{H_{ji}}^{k_2} F(H) = \sum_{\ell=0}^{2p+k_1+k_2} \sum_{\vec\sigma\in \{-1, 1\}^\ell}\sum_{\vec a\in \{x, y, i, j\}^\ell}\sum_{\vec b\in \{x, y, i, j\}^\ell} C_{\ell, \vec \sigma, \vec a, \vec b} \prod_{i=1}^\ell G_{a_i b_j}(\sigma_i),
\end{equation*}
where $C_{\ell, \vec \sigma \vec a, \vec b}$ is some constant, polynomial in $p$, and $\sigma_i$ represents whether we are considering the resolvent or its conjugate, i.e $G_{a_ib_i}(1) = G_{a_ib_i}$ and $G_{a_ib_i}(-1) = \overline{G_{a_ib_i}}$.

\textcolor{red}{1. prove by inductive argument}

\textcolor{red}{2. Stochastic bound $\prec 1$}
\subsection{Continuity Argument}
Having established the stochastic bound on the individual derivatives, we can now lay out the aforementioned continuity argument that will allows us to preserve control throughout the entire replacement. To start, define the following: 

\begin{equation*}
\begin{array}{c}
G_1 = G\left(H_{11}, \cdots, H_{ij}^G, \cdots, H_{NN}^G\right)\\
G_2 = G\left(H_{11}, \cdots,\tilde{t}_{ij}^G H_{ij}^G, \cdots, H_{NN}^G\right)\\
G_3 = G\left(H_{11}, \cdots,\tilde{t}_{ij} H_{ij}^G, \cdots, H_{NN}^G\right)
\end{array},
\end{equation*}
where $\tilde{t}_{ij}^G$ and $\tilde{t}_{ij}$ are the replacement variables at step $ij$ for this argument, and as such are different than the ones we used in our Lindeberg argument. By the matrix identity $(2.4)$, we have the following: 
\begin{equation*}
G_1 = G_2 + G_1\left(cH_{ij}^Ge_i^*e_j+ \tilde{c} H_{ji}^G e_j^*e_i\right)G_2
\end{equation*}
\textcolor{red}{have to do the same with the other terms? have to get an extra H}

$$(G_2)_{xy} = (G_1)_{xy} + cH_{ij}^G (G_1)_{xi}(G_2)_{jy}+\tilde{c}H_{ij}^G (G_1)_{xj}(G_2)_{iy}$$
Given $(8.2)$, we have $(G_1)_{xy}\prec 1$, and as such: 
$$|(G_2)_{xy}|\prec 1 + W^{-\frac{1}{2}}|(G_2)_{jy}|+W^{-1/2}|(G_2)_{iy}|$$
Let $\Xi = \max_{x, y}|(G_2)_{xy}|$, then: $$\Xi \prec 1 + W^{-1/2}\Xi,$$
hence $\Xi\prec 1$ (\textcolor{red}{requires justification for the $N^\epsilon$ exponents from the stochastic domination}. 

\textcolor{red}{How does that complete the argument?}




\subsection{Five Moment Theorem}
Let the LHS and RHS be $\mathcal{P}$ and $\mathrm{Q}$, respectively. We want $\mathcal{Q}\leq W^C$ for some $C \lesssim 1$ , s.t. $\mathcal{Q}\prec 1$ (\textcolor{red}{definition? - revisit}). \\
...
Then, by applying the triangle inequality to the telescoping sum of replacements, and accounting for the $NW$ non-zero entries in the band, we have: 
 \begin{equation*} \left\vert \mathbb{E}\left(F(H)\right) - \mathbb{E}\left(F(H^G)\right)\right\vert\lesssim NWW^{-3+\epsilon} = NW^{-2+\epsilon}\tag{8.35?}
 \end{equation*}
...
\noindent Bounding $(8.3)$ by the latter condition gives us:
...
\noindent Hence, in order for our asymptotics to work, we need a negative exponent, i.e: $$-2+\epsilon + \frac{2}{1+2\beta}<0$$
  $$0<\epsilon <2-\frac{2}{1+2\beta} = \frac{4\beta}{1+2\beta},$$
  whereas since $\beta>0\Rightarrow \frac{4\beta}{1+2\beta}$, we can always pick such an $\epsilon>0$ by the density of the rationals. Now, the only missing piece is to justify the assumption we made earlier, namely $\mathcal{Q}\prec 1$. For this purpose, we need to establish stochastic control of the resolvent at each step of the 
