...
\noindent My high school teacher, Elena Kiselova, for sowing the seeds of trees I am now sitting in the shade of. \\
...
This means that we know need to establish an equivalent bound for the first term on the RHS. Assume $\exists \mathcal{B}$ an event for which the following is true:
...
We can use the trace inequality to show that: 
...
$$\Rightarrow \frac{1}{4-2\kappa}\int dE \frac{1}{N}\sum_{\alpha\in \mathcal{A}_{\epsilon, \ell}^C}\frac{\eta}{\im m(z)|\lambda_\alpha-E-i\eta|^2}\leq C\sqrt{\epsilon}+O(N^{-c}),$$
