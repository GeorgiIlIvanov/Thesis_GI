...
\section{Flow Method}
\label{sec:flow}
Consider the Brownian perturbation of the standard deviation matrix defined as $H(t)_{xy} = \sqrt{S_{xy}}b(t)_{xy}$, where $b(t)\sim \mathcal{N}(0, t)$ represents the Brownian motion. For this system, the matrix resolvent is augmented as $G(t) = (H(t)-w(t))^{-1}$ , where: $$w(t) = -\frac{1}{m(z)}-tm(z) = z+(1-t)m(z)$$
...
\textcolor{red}{Applying Itô's formula}(show it explicitly) to \( F(t)_{xy} \), we have:
\[
...
\label{diffusion}
Recall our time-dependent $T$-equation $T(t)$ and diffusion profile $\Theta_t$. The $T$-matrix and diffusion profile $\Theta$ are just the latter two for $t=1$, i.e: $$T(z)_{ab} = T_{ab} = \sum_{x,y}S_{ax}^{\frac{1}{2}} |G_{xy}|^2S_{yb}^{\frac{1}{2}}$$
