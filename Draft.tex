...
We can pick $D_0$, s.t. the stochastically dominating term is $W^{-3+\epsilon}$, s.t. we get $\mathcal{P}+\mathcal{P}^G\lesssim W^{-3+\epsilon}$, i.e: $$\left\vert \mathbb{E}\left(F(H_{ij}\right)\right\vert$$
...
\section{Combined formula}
$$\sum_{\beta=0}^{2p+d}\sum_{\vec i, \vec j, \vec \sigma} c_{\vec i, \vec j, \vec \sigma} \prod_{\alpha =1}^\beta G_{\vec j_{\alpha\alpha}}$$
