...
$}\end{equation*}
\end{center}
\noindent To estimate this graph, we will bound all loops $\textcolor{blue}{\circlearrowright}$ and $x\textcolor{red}{\rightarrow} y$ using estimate $(2)$ which yields in total $O(W^{\delta_\sto/5}W^{-2}\eta_s^{-1})$. Then, we sum the wavy lines over $\beta, \alpha, v, u, \delta$ in that order, which is $O(1)$ by estimate 5. Then the combined bound is:
...
\noindent Here, we will repeat an equivalent procedure as above, bounding the loop at $\delta$ and the three edges $x\textcolor{red}{\rightarrow}\alpha$, $\beta\textcolor{blue}{\rightarrow}\alpha$ and $\beta\textcolor{red}{\rightarrow}y$ , getting again a bound $O(W^{\delta_\sto/5}W^{-2}\eta_s^{-1})$. Then, we can sum the wavy lines with estimate $(5)$, getting an equivalent bound as above:
