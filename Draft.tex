...
\item $\mathbb{E}\left[H_{ij}^\ell \left(\overline{H}_{ij}\right)^{k-\ell}\right] = \mathbb{E}\left[\left(H_{ij}^G\right)^\ell \left(\overline{H}_{ij}^G\right)^{k-\ell}\right], \quad 0\leq \ell\leq k\leq 5$
...
$$\frac{d^k}{dt^k} F(tH_{ij}) = \sum_{\ell=0}^k \binom{k}{\ell}\partial_{H_{ij}}^\ell \partial_{H_{ji}}^{k-\ell} F(tH_{ij})H_{ij}^\ell \left(\overline{H_{ij}}\right)^{k-\ell}$$
...
\begin{equation*}
\frac{d^{k+1}}{dt^{k+1}} F(tH_{ij}) = \sum_{\ell=0}^{k+1} \binom{k+1}{\ell}\partial_{H_{ij}}^\ell \partial_{H_{ji}}^{k+1-\ell} F(tH_{ij})H_{ij}^\ell \left(\overline{H_{ij}}\right)^{k+1-\ell}\tag{8.4}
\end{equation*}
...
where $F(sH_{ij})=F(0)$ for $s=0$ represents setting the $H_{ij}$ and $H_{ji}$ to zero. Similarly, $t_{ij}^G\in[0, 1]$, s.t.:
...
\end{array}
\end{equation*}
$$ =\sum_{k=1}^5 \frac{1}{k!} \left[\left. \frac{d^k}{dt^k}F(tH_{ij})\right\vert_{t=0}-\left. \frac{d^k}{dt^k}F(tH_{ij}^G)\right\vert_{t=0}\right\vert +\frac{1}{720} \left[\left.\frac{d^6}{dt^6}F(tH_{ij})\right\vert_{t=t_{ij}}-\left.\frac{d^6}{dt^6}F(tH_{ij})\right\vert_{t=t_{ij}^G}\right]$$
Focusing on the first term, we can u
We can now recall our formula from before, s.t. our first term becomes:
$$\sum_{k=1}^t \left[\right]$$

$$ \sum_{k=1}^5\frac{1}{k!}\sum_{\ell = 0}^k c_{\ell, k} \partial_{H_{ij}}^\ell \partial_{H_{ji}}^{k-\ell} F(0)\left\{H_{ij}^\ell \left(\overline{H}_{ij} \right)^{k-\ell}- \left(H_{ij}^G\right)^\ell \left(\overline{H}_{ij}^G\right)^{k-\ell}\right\}+$$
