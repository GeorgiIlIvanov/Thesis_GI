...
\end{array},
\end{equation*}
where $\tilde{t}_{ij}^G$ and $\tilde{t}_{ij}$ are the replacement variables $\in [0, 1]$ at step $ij$ for this argument, different than the ones we used in our Lindeberg argument. By the matrix identity $(2.4)$ for $A = H_2-z$ and $B = H_1-H_2$, we have the following: 
$$G_1 = G_2 - G_2\left(H_1-H_2\right)G_1$$
$$\Rightarrow G_2 = G_1+G_2\left(H_1-H_2\right)G_1$$

\noindent Given that $H_2$ and $H_1$ differ only in their $(ij)$th and $(ji)$th component, we can represent their difference as the sum of two matrices with the only non-zero entries being the latter, which by definition of the orthonormal basis is $e_i^*e_j$ and $e_j^*e_i$ respectively. Then, for $c = 1-\tilde{t}_{ij}^G\in [0, 1]$, we have:
\begin{equation*}
...
Let $\Xi_2 = \max_{x, y}|(G_2)_{xy}|$. We can rewrite the expression above as: 
\begin{equation*}\Xi \prec 1 + W^{-1/2}\Xi_2\tag{8.7}\end{equation*}

\noindent Now, let $\epsilon, D>0$ be arbitrary. Given that we work in the context of the delocalization, we have $W\gg N^{\frac{1}{2}+\beta} = N^{\frac{1+2\beta}{2}}\Rightarrow W^{-1/2}\ll N^{-\frac{1+2\beta}{4}}$. Hence, let us choose $\delta = \min \left(1, \frac{2\beta+1}{4\epsilon}\right)$. 
By Def.1 of stochastic domination, from $(8.7)$, we have that: $$\sup_{s\in S_N}\mathbb{P}\left(\Xi_N(s)>N^{\delta\epsilon}(1+W^{-1/2}\Xi_N(s))\right)<N^{-D}$$
...
By our choice of $\delta$, we clearly have that $$N^{\delta\varepsilon}W^{-1/2} \ll N^{\delta\varepsilon} \cdot N^{-1/4-\beta/2} = N^{\delta\varepsilon-1/4-\beta/2} \ll 1$$
This means that $\exists N_0(\beta, \epsilon)$, s.t $\forall N\geq N_0$, we have that $\frac{N^{\delta\epsilon}}{(1-N^{\delta\epsilon} W^{-1/2})}>N^{\delta\epsilon}$. This means that for all $N\geq N_0$, the latter event is a subset: $$\left\{\Xi_N(s)>\frac{N^{\delta\epsilon}}{(1-N^{\delta\epsilon} W^{-1/2})}\right\}\subset \left\{\Xi_N(s)>N^{\delta \epsilon}\right\}$$
Define 



hence $\Xi_2\prec 1$.
Notice that the approach is equivalent for $\Xi_3 = \max_{x, y}|(G_3)_{xy}|$, i.e WLOG, we have that $\Xi_3\prec 1$ as well. 

 
\textcolor{red}{complete the continuity argument}. 
