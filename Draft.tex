...
\begin{equation*}
\partial_{H_{ij}}^{k_1}\partial_{H_{ji}}^{k_2} F(H)= \sum_{\ell=0}^{2p+k_1+k_2} \sum_{\vec\sigma\in \{-1,0, 1\}^\ell}\sum_{\vec a\in \{x, y, i, j\}^\ell}\sum_{\vec b\in \{x, y, i, j\}^\ell} C_{\ell, \vec \sigma, \vec a, \vec b} \prod_{i=1}^\ell \tilde{G}_{a_i b_j}(\sigma_i)\tag{8.6},
\end{equation*}
...
\begin{itemize}
\item If $a_i=x \wedge b_i=y$, then $\tilde{G}_{a_i, b_i}(1) = W_{xy}$,  $\tilde{G}_{a_i, b_i}(-1) = \overline{W_{xy}}$ and $\tilde{G}_{a_ib_i}(0) = |W_{xy}|$. 
\item For $(a_i=x\wedge b_i=y)^C$, we have $\tilde G_{a_ib_i}(1) = G_{a_ib_i}$ and $\tilde G_{a_ib_i}(-1) = \overline{G_{a_ib_i}}$.
 \end{itemize} 
Note that we included a separate index that would account for $|W_{xy}|$ by itself, given that it figures as an element, separate from $W_{xy}$ and $\overline{W_{xy}}$. We can trivially see that the base case is satisfied by the definition of $F(H)$ and the first derivative we proved. Let us assume $(8.6)$ for the induction hypothesis. Now, we want to prove the cases for $k_1+1$ and $k_2+1$. Starting with the first one, we can differentiate $(8.6)$, s.t. by linearity and the product rule: 


$$\partial_{H_{ij}}^{k_1+1}\partial_{H_{ji}}^{k_2} F(H) = \partial_{H_{ij}}\left(\partial_{H_{ij}}^{k_1}\partial_{H_{ji}}^{k_2} F(H)\right) = \sum_{\ell=0}^{2p+k_1+k_2} \sum_{\vec\sigma}\sum_{\vec a}\sum_{\vec b} C_{\ell, \vec \sigma, \vec a, \vec b}\  \partial_{H_{ij}}\left(\prod_{i=1}^\ell\left\vert \tilde{G}_{a_i b_i}(\sigma_i)\right\vert\right)=$$
$$=\sum_{\ell=0}^{2p+k_1+k_2} \sum_{\vec\sigma}\sum_{\vec a}\sum_{\vec b}C_{\ell, \vec \sigma, \vec a, \vec b}\ \sum_{k=1}^\ell \left(\partial_{H_{ij}}\left\vert \tilde{G}_{a_k b_k}(\sigma_k)\right\vert \prod_{i=1,i\neq k}^\ell\left\vert \tilde{G}_{a_i b_i}(\sigma_i)\right\vert\right) $$
We now need to calculate $\partial_{H_{ij}}\left\vert \tilde{G}_{a_k b_k}(\sigma_k)\right\vert$, where the cases are two, as delineated above:
\begin{itemize}
\item If $a_k=x \wedge b_k=y$, then $\tilde{G}_{a_k b_k}(\sigma_k)$ is either $W_{xy}$ or $\overline{W_{xy}}$. Using the chain rule and the resolvent derivatives $\partial_{H_{ij}}G_{xy} = -G_{xi}G_{jy}$, we have the following (or its conjugate): $$\partial_{H_{ij}}\left\vert W_{xy}\right\vert = -\frac{1}{\left\vert W_{xy}\right\vert}\text{Re}(G_{xi}G_{jy}\overline{W_{xy}})$$

\item If $(a_k=x\wedge b_k=y)^C$, then $\tilde{G}_{a_k b_k}(\sigma_k)$ is either $G_{a_k b_k}$ or $\overline{G_{a_k b_k}}$. By the same token, we have (or its conjugate): $$\partial_{H_{ij}}\left\vert G_{a_k b_k}\right\vert = -\frac{1}{\left\vert G_{a_k b_k}\right\vert}\text{Re}(G_{a_k, i}G_{j, b_k}\overline{G_{a_k b_k}})$$
\end{itemize}
We can use the fact that $|\text{Re}(z)|\leq |z|$ to bound our derivative term using the triangle inequality: 
$$\left|\partial_{H_{ij}}\left\vert \tilde{G}_{a_k b_k}(\sigma_k)\right\vert\right| \leq \frac{|\tilde{D}_{a_k b_k}||\tilde{G}_{a_k b_k}(\sigma_k)|}{|\tilde{G}_{a_k b_k}(-\sigma_k)|} = |\tilde{D}_{a_k b_k}|,$$
where $\tilde{D}_{a_k b_k} = \tilde{G}_{xi}\tilde{G}_{jy}\vee \tilde{G}_{a_k i}\tilde{G}_{j b_k}$, since conjugates have the same absolute value. This means that: 

$$\partial_{H_{ij}}^{k_1+1}\partial_{H_{ji}}^{k_2} F(H) \leq \sum_{\ell=0}^{2p+k_1+k_2} \sum_{\vec\sigma}\sum_{\vec a}\sum_{\vec b}C_{\ell, \vec \sigma, \vec a, \vec b}\ \sum_{k=1}^\ell \left(\partial_{H_{ij}}\left\vert \tilde{G}_{a_k b_k}(\sigma_k)\right\vert \prod_{i=1,i\neq k}^\ell\left\vert \tilde{G}_{a_i b_i}(\sigma_i)\right\vert\right) \leq $$
$$ \leq \sum_{\ell=0}^{2p+k_1+k_2} \sum_{\vec\sigma}\sum_{\vec a}\sum_{\vec b}C_{\ell, \vec \sigma, \vec a, \vec b}\ \sum_{k=1}^\ell \left(|\tilde{D}_{a_k b_k}| \prod_{i=1,i\neq k}^\ell\left\vert \tilde{G}_{a_i b_i}(\sigma_i)\right\vert\right)$$
...
$$\partial_{H_{ij}}^{k_1+1}\partial_{H_{ji}}^{k_2} F(H) \leq \sum_{m=1}^{2p+k_1+k_2+1} \sum_{\vec\sigma'}\sum_{\vec a'}\sum_{\vec b'} C'_{m, \vec\sigma', \vec a', \vec b'} \prod_{i=1}^m\left\vert \tilde{G}_{a'_i b'_i}(\sigma'_i)\right\vert$$
This clearly matches the form of $(8.6)$. And since the proof for $\partial_{H_{ij}}^{k_1}\partial_{H_{ji}}^{k_2+1} F(H)$ follows WLOG, we have indeed completed the induction. With the completed bound on $\partial_{H_{ij}}^{k_1}\partial_{H_{ji}}^{k_2} F(H)$, we can now construct the continuity argument. 

\noindent  We will assume minimal resolvent bounds, namely that $|\tilde{G}_{k\ell}|\prec 1$ for all $k, \ell\in \{1, \cdots, N\}$. By definition of stochastic domination (\hyperref[def-1]{Def.1}), we have that for all $\epsilon_{a_i,b_i}>0$ and $D_{a_ib_i}>0$, $\exists N_{\epsilon_{a_ib_i}, D_{a_ib_i}}$ s.t.: $$\sup_{s\in S_N}\mathbb{P}\left(\left\vert\tilde{G}_{a_ib_i}\right\vert(\sigma_i)>N^{\epsilon_{a_ib_i}} \right)<N^{-D_{a_ib_i}}$$
By using the product rule of stochastic domination (\hyperref[lemma-a6]{Lemma A6}), we have that $\prod_{i=1}^\ell \left\vert \tilde{G}_{a_ib_i}(\sigma_i)\right\vert \prec 1^\ell = 1$, and as such $C_{\ell, \vec a, \vec b, \vec \sigma}\prod_{i=1}^\ell \left\vert \tilde{G}_{a_ib_i}(\sigma_i)\right\vert \prec 1^\ell = 1$. And since the sum is finite, where the number of terms is $\sum_{\ell=0}^{2p+k_1+k_2} 2^{\ell}\cdot 4^\ell\cdot 4^\ell = \sum_{\ell=0}^{2p+k_1+k_2}32^\ell$, by the finite sum rule of stochastic domination (\hyperref[lemma-A7]{Lemma A7}), we also have that $\sum_{\ell=0}^{2p+k_1+k_2} \sum_{\vec\sigma}\sum_{\vec a}\sum_{\vec b} |C_{\ell, \vec \sigma, \vec a, \vec b}| \prod_{i=1}^\ell\left\vert \tilde{G}_{a_i b_i}(\sigma_i)\right\vert\prec 1$. B

\subsection{Continuity Argument}
\noindent Having established the stochastic bound on the individual derivatives, we can now lay out the continuity argument that will allows us to preserve control throughout the entire replacement. To start, define the following: 
...
\end{array},
\end{equation*}
where $\tilde{t}_{ij}^G$ and $\tilde{t}_{ij}$ are the replacement variables at step $ij$ for this argument, and as such are different than the ones we used in our Lindeberg argument. By the matrix identity $(2.4)$ for $A = H_1-z$ and $B = H_2-H_1$, we have the following: 
$$G_2 = G_1 - G_1\left(H_2-H_1\right)G_2$$
$$\Rightarrow G_1 = G_2+G_1\left(H_2-H_1\right)G_2$$

\noindent Given that $H_2$ and $H_1$ differ only in their $(ij)$th and $(ji)$th component, we can represent that as the sum of two matrices with the only non-zero entries being the latter, which by definition of the orthonormal basis is $e_i^*e_j$ and $e_j^*e_i$ respectively. 
\begin{equation*}
