...
Since for any $s\in [0, 1]$ we have $\im w_s = \im w_t+(t-s)\im m(z)\geq \im w_t$
and \hyperref[3.1]{$\im m\asymp 1$}, then:

$$\im w_s = \eta+(1-s)\im m(z)\leq \eta+ \im m(z) = \im w_0\lesssim 1$$
By doing a change of variable for $\sigma = \im w_s$, we get: 
\begin{equation*}
|\E_t^{S, \sto}(z)_{ab}|\lesssim W^{2\delta_\sto} W^{-\frac{17}{8}-\nu}\int_{\eta_t}^{\im w_0}\sigma^{-3}d\sigma + \eta_t^{-2} W^{2\delta_\sto} W^{-\frac{17}{8}-\nu}\int_{\eta_t}^{\im w_0}\sigma^{-1}d\sigma \lesssim
\end{equation*}
\begin{equation*}
