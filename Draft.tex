...
following from the Schwarz inequality, since $|G|_{xx'}^2|G|^2_{x'x}=||G|_{xx'}^2|^2$ by virtue of $|G|^2$ being Hermitian. On the other hand, by the \hyperref[ward]{Ward identity}, \hyperref[lemma-a2]{Lemma A.2}, the stopping time $s\leq \tau_\sto$ and the fact we showed earlier of $(t-s)\lesssim \eta_s$, we get:
$$4 \sum_{x}|G|_{xx}^4 |(\Theta_t S^{\frac{1}{2}})_{ax}|^2=4 |\text{Im}w_s|^{-2}\sum_x|\text{Im}G_{xx}|^2|(\Theta_t S^{\frac{1}{2}})_{ax}|^2\leq 4 |\text{Im}w_s|^{-2}|\max_{k}\text{Im}G_{kk}|^2\sum_x|(\Theta_t S^{\frac{1}{2}})_{ax}|^2$$
...
By the same logic, we have that: $$\sum_{u, y}\Xi_{yy}^u=\sum_{u,y}\sum_{x,x'} \overline{G}_{xy} G_{x'y'} G_{xu} \overline{G}_{x'u} S^{\frac{1}{2}}_{ax} S^{\frac{1}{2}}_{ax'} = \sum_{x, x'}|G|^2_{xx'}|{G}|^2_{x'x} S^{\frac{1}{2}}_{ax} S^{\frac{1}{2}}_{ax'}\leq $$
...
Lastly, we need to bound the other two matrices, which we can do by using the fact that the diagonal matrix $S^u$ is of the same order as \hyperref[cor-a3]{Corollary A.3}, i.e by the same approach: 
$$\|\Upsilon^u\|_{op}\leq \max_{\alpha, \beta}|(\Theta_t S^{1/2})|^2 W^{-1}|\text{Im}w_s|^{-2}\lesssim W^{-1}|\text{Im}w_s|^{-2}|\text{Im}w_t|^{-1}$$
$$\|\Gamma^u\|_{op}\leq\max_{\alpha,\beta}|S_{\alpha\beta}^{1/2}|^2W^{-1}|\text{Im}w_s|^{-2}\lesssim W^{-3}|\text{Im}w_s|^{-2}$$
