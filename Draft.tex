...
By looking at the general form of our target $\left\vert \partial_{H_{ij}}^\ell \partial_{H_{ji}}^{k-\ell} F(tH_{ij})\right\vert$ , we can glean that a good place to start would be with the calculation of $\partial_{H_{xy}}^\ell F(tH_{xy})$. Given our definition of the interpolation $tH_{xy}$, we can tell in advance that term $t$ will only figure as a multiple after applying the chain rule, so we can focus exclusively on $\partial_{H_{xy}}^\ell F(tH_{xy})$. Recall our $(8.4)$ definition of $F(H)$definition Let us begin by abstracting slightly our notation, defining $W_{ij}$ as : 
$$F(H) = \left\vert G_{ij}\right\vert
$$

is the result of has several derivatives w.r.t to different entries, let us 
