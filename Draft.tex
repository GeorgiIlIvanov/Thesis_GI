...
Up to this point, our work on non-mean field models, specifically band matrices, has been focused on the case with Gaussian entries. Given that the delocalization behavior is conjectured to extend beyond the latter case, our last section has has t\textcolor{red}{Finish sentence}
...
\subsection{Four Moment Theorem for Green's function}
 Let $M_n, M_n'$ be two Wigner random matrices satisfying \textcolor{red}{\textbf{C0}} (define it) that match to order 4 off the diagonal and to order 2 on the diagonal for some sufficiently large $C_0$. Let $z = E + i\eta$ for some $E \in \mathbb{R}$ and some $\eta > 0$. We assume the level repulsion hypothesis that for any $c > 0$, one has with high probability that
\begin{equation}
...
\subsection{Result}
At the root of the Lindeberg exchange strategy is to establish control at each step of the replacement. The matrix-equivalent for this is to swap one entry $H_{ij}$ at a time (up to the Hermitian control) with its matching Gaussian in the following sense: 
\begin{boxt}{Conditions}
\begin{enumerate}[label=(8.\arabic*)]
\item $\mathbb{E}\left[H_{ij}^\ell \left(\overline{H}_{ij}\right)^{k-\ell}\right] = \mathbb{E}\left[\left(H_{ij}^G\right)^\ell \left(\overline{H}_{ij}^G\right)^{k-\ell}\right]$
\item $\forall q\geq 1, \quad \mathbb{E}\left[H_{xy}\right]^{2q}\prec W^{-q}$
\end{enumerate}
\end{boxt}
...
\textcolor{red}{elaborate on conditions}. In order to apply the Lindeberg expansion strategy to our case, let us consider the following function: $$F(H) = \left\vert
(H-z)_{ij}^{-1} - m(z)S_{ij}
\right\vert ^{2p} = \left\vert 
G_{ij}(z)-m(z)S_{ij}
\right\vert ^{2p} = |W_{ij}|^{2p}$$
\textcolor{red}{give intuition}

\noindent Consider the replacement at stage $ij$ for $t\in [0, 1]$: 

$$F(tH_{ij}) = F(H_{11}^G, \cdots, H_{i, j-1}^G, tH_{ij}, H_{i, j+1}, \cdots H_{NN})$$
 By the chain rule: $$\partial_t F(tH_{ij}) = \partial_{H_{ij}} F(tH_{ij})H_{ij} + \partial_{H_{ji}} F(tH_{ij})\overline{H_{ij}}$$
(\textcolor{red}{prove by induction? - also justify the first formula} - Similarly, we have: 
$$\frac{d^k}{dt^k} F(tH_{ij}) = \sum_{k=0}^k c_{\ell, k}\partial_{H_{ij}}^\ell \partial_{H_{ji}}^{k-\ell} F(tH_{ij})H_{ij}^\ell \left(\overline{H_{ij}}\right)^{k-\ell}$$

\noindent By Taylor expansion around $t=0$: 
$$F(H_{ij}) =F(0) + \sum_{k=1}^5 \frac{1}{k!} \frac{d^k}{dt^k}\left. F(tH_{ij})\right\vert_{t=0?1}+ \frac{1}{720} \frac{d^6}{dt^6}\left.F(tH_{ij})\right\vert_{t=t_{ij}},$$
where by Taylor's theorem? $t_{ij}\in [0, 1]$ reflects all of the remaining correlations. Similarly, 

$$F(H_{ij}^G) =F(0) + \sum_{k=1}^5 \frac{1}{k!} \frac{d^k}{dt^k}\left. F(tH_{ij}^G)\right\vert_{t=0?1}+ \frac{1}{720} \frac{d^6}{dt^6}\left.F(tH_{ij}^G)\right\vert_{t=t_{ij}^G},$$
for another such $t_{ij}^G\in[0, 1]$.
Then, let us take their difference (\textcolor{red}{verify the matrix identity }$\{--\}$:
$$F(H_{ij}) - F(H_{ij}^G) = \sum_{k=1}^5\frac{1}{k!}\sum_{\ell = 0}^k c_{\ell, k} \partial_{H_{ij}}^\ell \partial_{H_{ji}}^{k-\ell} F(0)\left\{H_{ij}^\ell \left(\overline{H}_{ij} \right)^{k-\ell}- \left(H_{ij}^G\right)^\ell \left(\overline{H}_{ij}^G\right)^{k-\ell}\right\}+$$
$$+\frac{1}{720} \left[\left.\frac{d^6}{dt^6}F(tH_{ij})\right\vert_{t=t_{ij}}-\left.\frac{d^6}{dt^6}F(tH_{ij})\right\vert_{t=t_{ij}^G}\right]$$
By the moment matching condition $(8.1)$, we have: $$\mathbb{E}\left\vert F(H_{ij} - F(H_{ij}^G)\right\vert\lesssim \mathbb{E}\left\vert\left.\frac{d^6}{dt^6}F(tH_{ij})\right\vert_{t=t_{ij}}\right\vert+\mathbb{E}\left\vert\left.\frac{d^6}{dt^6}F(tH_{ij})\right\vert_{t=t_{ij}^G}\right\vert$$
Using the formula we found earlier (\textcolor{red}{write out the cancellation by absolute value}):
$$\mathbb{E}\left\vert\left.\frac{d^6}{dt^6}F(tH_{ij})\right\vert_{t=t_{ij}}\right\vert\lesssim \sum_{k=0}^6 |H_{ij}|^k \left\vert \partial_{H_{ij}}^\ell \partial_{H_{ji}}^{k-\ell} F(tH_{ij})|_{t=t_{ij}} \right\vert$$
...
Then, by applying the triangle inequality to the telescoping sum of replacements, and accounting for the $NW$ non-zero entries in the band, we have: 
 \begin{equation*} \left\vert \mathbb{E}\left(F(H)\right) - \mathbb{E}\left(F(H^G)\right)\right\vert\lesssim NWW^{-3+\epsilon} = NW^{-2+\epsilon}\tag{8.3}
 \end{equation*}
...
The second term appears because we're dealing with complex numbers, and $|W_{ij}|^2 = W_{ij} \cdot W_{ij}^*$. T
