...
\begin{equation*}
\partial_{H_{ij}}^{k_1}\partial_{H_{ji}}^{k_2} F(H)\leq \sum_{\ell=0}^{2p+k_1+k_2} \sum_{\vec\sigma\in \{-1, 1\}^\ell}\sum_{\vec a\in \{x, y, i, j\}^\ell}\sum_{\vec b\in \{x, y, i, j\}^\ell} C_{\ell, \vec \sigma, \vec a, \vec b} \prod_{i=1}^\ell\left\vert  \tilde{G}_{a_i b_j}(\sigma_i)\right\vert\tag{8.6},
\end{equation*}
...
\item For $(a_i=x\wedge b_i=y)^C$, we have $\tilde G_{a_ib_i}(1) = G_{a_ib_i}$ and $\tilde G_{a_ib_i}(-1) = \overline{G_{a_ib_i}}$.
 \end{itemize} 
Note that $(8.6)$ would have been an equality (without absolute values around $\tilde G$), had we included a separate index that would account for $|W_{xy}|$ by itself, given that it figures as an element, separate from $W_{xy}$ and $\overline{W_{xy}}$. But again, given that we are only interested in a rough stochastic estimate, we don't need to be as careful, which is why we use an upper bound, provided by the triangle inequality and the fact that $x\leq |x|$. We can trivially see that the base case is satisfied by the definition of $F(H)$ and the first derivative we proved. Let us assume $(8.6)$ for the induction hypothesis. Now, we want to prove the cases for $k_1+1$ and $k_2+1$. Starting with the first one, we can differentiate $(8.6)$, s.t. by linearity and the product rule: 


$$\partial_{H_{ij}}^{k_1+1}\partial_{H_{ji}}^{k_2} F(H) = \partial_{H_{ij}}\left(\partial_{H_{ij}}^{k_1}\partial_{H_{ji}}^{k_2} F(H)\right) \leq \sum_{\ell=0}^{2p+k_1+k_2} \sum_{\vec\sigma}\sum_{\vec a}\sum_{\vec b} C_{\ell, \vec \sigma, \vec a, \vec b}\  \partial_{H_{ij}}\left(\prod_{i=1}^\ell\left\vert \tilde{G}_{a_i b_i}(\sigma_i)\right\vert\right)=$$
...
This clearly matches the form of $(8.6)$. And since the proof for $\partial_{H_{ij}}^{k_1}\partial_{H_{ji}}^{k_2+1} F(H)$ follows WLOG, we have indeed completed the induction. 


\subsection{Continuity Argument}
With the completed bound on the functional expression of derivatives, we can now construct the continuity argument. We will assume minimal resolvent bounds, namely that $|\tilde{G}_{k\ell}|\prec 1$ for all $k, \ell\in \{1, \cdots, N\}$. 

Having established the stochastic bound on the individual derivatives, we can now lay out the continuity argument that will allows us to preserve control throughout the entire replacement. To start, define the following: 
