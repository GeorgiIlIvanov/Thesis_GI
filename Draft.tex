...
\end{equation*}
\end{center}
Here, we eliminate first the edges  $\beta\textcolor{red}{\rightarrow}\alpha$ and $\alpha\textcolor{blue}{\rightarrow} y$ using estimate $(E2)$, giving us a $O(W^{\delta_\sto/10}W^{-1}\eta_s^{-1/2})$ term. Due to the edge from $x\textcolor{blue}{\rightarrow}\beta$, we are cannot sum $\alpha\leadsto\beta$ over $S_{ab}$ entries because it serves as a multiplicative term to the blue edge $x\textcolor{blue}{\rightarrow}\beta$. For this reason, we will apply the individual $S_{ab}\lesssim W^{-1}$ bound, s.t now summing over $x\textcolor{blue}{\rightarrow}\beta$ with estimate $(E1)$ gives us a combined bound $O(W^{\delta_\sto/20}W^{\frac{1}{2}+\epsilon}\eta_s^{-3/4})$. Finally, by summing over the remaining wavy edges, starting with $\alpha, v$ and then $u$, due to the $O(1)$ term (estimate $(E5)$), we get an estimate of the form: $\mathcal{G}_{30, s}\lesssim W^{3\delta_\sto/20}W^{-\frac{1}{2}+\epsilon}\eta_s^{-5/4}\times $
\begin{tikzpicture}[
    thick, % Line thickness
    wavy/.style={decorate, decoration={snake, amplitude=2pt, segment length=5pt}},
    ]
