...
$$\E_t^{M, \text{stop}}(z)=-\int_0^{t\wedge \tau_\sto} \{\text{Id} + (t-s)\Theta_t\} S^{1/2}dM(s)S^{1/2} \{\text{Id} + (t-s)\Theta_t\}= $$$$=-\int_0^{t\wedge \tau_\sto} \{\text{Id} + (t-s)\Theta_t\}\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\{\text{Id} + (t-s)\Theta_t\}-$$
