...
\noindent Given that $H_2$ and $H_1$ differ only in their $(ij)$th and $(ji)$th component, we can represent their difference as the sum of two matrices with the only non-zero entries being the latter, which by definition of the orthonormal basis is $e_i^*e_j$ and $e_j^*e_i$ respectively. Then, for $c_{12} = \tilde{t}_{ij}^G-1$, we have:
\begin{equation*}
G_1 = G_2 + G_1\left((cH_{ij}^Ge_i^*e_j+ cH_{ji}^G e_j^*e_i\right)G_2
\end{equation*}

$$\Rightarrow (G_2)_{xy} = (G_1)_{xy} + cH_{ij}^G (G_1)_{xi}(G_2)_{jy}+\tilde{c}H_{ij}^G (G_1)_{xj}(G_2)_{iy}$$
