...
$}\end{equation*}
\end{center}
\noindent To estimate this graph, we will bound all loops $\textcolor{blue}{\circlearrowright}$ and $x\textcolor{red}{\rightarrow} y$ using estimate $(2)$ which yields in total $O(W^{\delta_\sto/5}W^{-2}\eta_s^{-1})$. Then, we sum the wavy lines over $\beta, \alpha, v, u, \delta$ in that order, which is $O(1)$ by estimate 5. This means that we get a combined bound:
