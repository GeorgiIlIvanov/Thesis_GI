...
\end{array}\right.$
\end{center}
Observe that the latter subgraphs are almost identical to the one we calculate for the $\mathcal{G}_{i0, s}$ estimates with the addition of a solid line. This means that we have to multiply our $(6.1)$ bound by an additional term $W^{\frac{1}{2}+\epsilon}\eta_s^{-3/4}$ from estimate $(E1)$ (and the fact that $\eta_0\lesssim 1$):
$$\max_{i=1, \cdots, 4}| \mathcal{G}_{i1, s}|\lesssim W^{\delta_\sto/4}W^{-2+2\epsilon}\eta_t^{-3/2}\eta_s^{-1/2}+W^{\delta_\sto/4+\epsilon}W^{-2}\eta_t^{-1}\eta_s^{-1}$$
$$\Rightarrow \max_{i=1, \cdots, 4}\int_{0}^{t}|\mathcal{G}_{i1, a}|ds\lesssim W^{\delta_\sto/4}W^{-2+2\epsilon}\eta_t^{-3/2}\int_{\eta_t}^{\eta_0}\sigma^{-1/2}d\sigma + W^{\delta_\sto/4+\epsilon}W^{-2}\eta_t^{-1}\int_{\eta_t}^{\eta_0}\sigma^{-1}d\sigma\lesssim$$
$$\lesssim W^{\delta_\sto/4}W^{-2+2\epsilon}\eta_t^{-3/2}\eta_0^{-1}-W^{\delta_\sto/4}W^{-2+2\epsilon}\eta_t^{-1}+$$
$$+W^{\delta_\sto/4+\epsilon}W^{-2}\eta_t^{-1}\ln \eta_0-W^{\delta_\sto/4+\epsilon}W^{-2}\eta_t^{-1}\ln \eta_t$$
$$\lesssim W^{\delta'}W^{-2}\eta_t^{-3/2}\lesssim W^{-\frac{3}{4}}\eta_t^{-1}W^{-1}\eta_t^{-\frac{1}{2}},$$
since $\delta' = \frac{\delta_\sto}{4}+2\epsilon>0$ is arbitrarily small. 
\newpage
