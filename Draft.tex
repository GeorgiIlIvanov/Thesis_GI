...
This means in fact that we can identify an integrating factor $U(t, s)$ in order to use Itô's lemma. Observe that: 
...
This is equivalent to $\partial_t U(t, s) = \Theta_t U(t, s)$, which makes $U(t, s)$ our evolution operator, since also $U(t, t) = \text{Id}$. Observe then that we can define the following integral form: $\E_t(z) = \int_0^t U(t, s) W(s) U(t, s)ds,$
where $W(s)$ includes our non-linear stochastic terms. 
 
\textcolor{red}{Integrating $\E_t(z)$ over $s$}:
$$\E_t(z)=\int_0^t \{\text{Id} + (t-s)\Theta_t\}\left\{\E^2(_sz)ds -S^{1/2} dM(s)S^{1/2}+  S^{1/2} \Omega(s) S^{1/2}\right\} \{\text{Id} + (t-s)\Theta_t\}ds,$$
...
$$\int_0^{t\wedge \tau_\sto} \{\text{Id} + (t-s)\Theta_t\} S^{1/2}dM(s)S^{1/2} \{\text{Id} + (t-s)\Theta_t\}= $$$$=\int_0^{t\wedge \tau_\sto} \{\text{Id} + (t-s)\Theta_t\}\sum_{u, v}\left[\overline{G_{xy}}G_{xu}dH_{uv}(t)G_{vy}\right]\{\text{Id} + (t-s)\Theta_t\}+$$
$$+ \int_0^{t\wedge \tau_\sto} \{\text{Id} + (t-s)\Theta_t\}\sum_{u, v}\left[G_{xy}\overline{G_{xu}dH_{uv}(t)}\overline{G_{vy}}\right]\{\text{Id} + (t-s)\Theta_t\}$$
...
We can bound $\max_{a, b}|\E_s(z)_{ab}^2|ds$ by using the definition of our stopping time. Since $s\leq \tau_\sto = \tau_{\sto, 1}\wedge \tau_{\sto, 2}$, by property of $\tau_{\sto, 1}$ and given a \textcolor{red}{bound $N\leq W^{\frac{11}{8}-\nu}$}, we have: $$\max_{a, b}|\E_s(z)_{ab}|<W^{\delta_\sto} W^{-\frac{3}{4}} \eta_s^{-1}\cdot W^{-1}\eta_s^{-\frac{1}{2}}$$
...
In this context, since \textcolor{red}{$\eta_t \geq \eta\geq W^{-3/4}$}, the first term is: $$W^{2\delta_\sto} W^{-\frac{3}{8}-\eta}\eta_t^{-\frac{1}{2}}\log \eta_t^{-1}\lesssim W^{2\delta_\sto-\nu}\log W,$$
...
\begin{boxt}{Lemma A.1.2} \label{lemma-a12} $ \im w_s = \im w_t + (t-s)\im m(z)$ \\
...
\textbf{Lemma A.2} \label{lemma-a2} - Lemma 23 \cite{bandSDE}

\textbf{Corollary A.3} \label{cor-a3} - Expression $(3.7)$ in \cite{bandSDE}
